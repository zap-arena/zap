...
# Progressive ("Code War") contests only attach is_progressive chain problems.
SEED_PROGRESSIVE_CONTESTS = {
    "Seed Code War Contest": (-1, 3),
}
...
def seed_problems(db, admin: models.User) -> list[models.Problem]:
    ...
    for item in parsed:
        ...
        problem.is_progressive = item["isProgressive"]
        ...
        db.query(models.ProblemStage).filter(models.ProblemStage.problem_id == problem.id).delete()
...
def seed_contests(db, admin: models.User, problems: list[models.Problem]) -> dict[str, models.Contest]:
    ...
    base = now()
    standard_problems = [p for p in problems if not p.is_progressive]
    progressive_problems = [p for p in problems if p.is_progressive]

    def build(name: str, start_h: int, end_h: int, mode: str, attach: list[models.Problem]):
        ...
        contest.instructions = (
            ...
            + ("Solve each chain stage to unlock the next, harder enhancement." if mode == "progressive"
               else "Solve as many problems as you can before the timer runs out.")
        )
        ...
        for order, problem in enumerate(attach):
    ...
    for name, (start_h, end_h) in SEED_CONTESTS.items():
        build(name, start_h, end_h, "standard", standard_problems[:PROBLEMS_PER_CONTEST])

    for name, (start_h, end_h) in SEED_PROGRESSIVE_CONTESTS.items():
        if progressive_problems:
            build(name, start_h, end_h, "progressive", progressive_problems)
...
def main() -> int:
    ...
    try:
        ...
        submissions = seed_participation(db, contests, users, [p for p in problems if not p.is_progressive], not args.no_submissions)
        db.commit()
    except Exception:
        db.rollback()
        raise
