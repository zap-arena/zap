...
export const accentHex = (a: Accent): string => hslToHex(a.dot.h, a.dot.s, a.dot.l);
...
const applyAccent = (a: Accent) => {
  ...
  root.style.setProperty('--shadow-glow', `0 0 30px hsl(${h} ${s}% ${l}% / 0.2)`);
  localStorage.setItem(ACCENT_KEY, a.id);
};

const getInitialAccent = (): Accent => {
  const id = typeof window === 'undefined' ? null : localStorage.getItem(ACCENT_KEY);
...
export const accentStore = {
  ...
  subscribe: (fn: () => void) => { accentListeners.add(fn); return () => { accentListeners.delete(fn); }; },
};

export const useAccent = () => {
  const [accent, setAccent] = useState<Accent>(accentStore.get());
  useEffect(() => accentStore.subscribe(() => setAccent(accentStore.get())), []);
