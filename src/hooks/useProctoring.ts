...
const BLOCK_MESSAGES: Partial<Record<ProctorEventType, BlockedAction>> = {
  COPY_BLOCKED: { title: 'Copying is disabled', message: 'Copying content is not allowed during the contest. This action has been recorded.' },
  CUT_BLOCKED: { title: 'Cutting is disabled', message: 'Cutting content is not allowed during the contest. This action has been recorded.' },
  PASTE_BLOCKED: { title: 'Pasting is disabled', message: 'Pasting content is not allowed during the contest. Type your solution in the editor.' },
  ESCAPE_PRESSED: { title: 'Escape is disabled', message: 'The Escape key is disabled during the contest. Use the Finish button to end your attempt.' },
  CONTEXT_MENU_BLOCKED: { title: 'Right-click is disabled', message: 'The context menu is not available during the contest.' },
  DEVTOOLS_ATTEMPT: { title: 'Developer tools are disabled', message: 'Opening developer tools is not allowed during the contest. This attempt has been recorded.' },
};
...
export function useProctoring(contestId: string | undefined, active: boolean, problemId: () => string | undefined, allowClipboard = false) {
  ...
  const report = useCallback((type: ProctorEventType, metadata: Record<string, unknown> = {}) => {
  ...
  useEffect(() => {
    ...
    const onFullscreenChange = () => {
      ...
      setIsFullscreen(now);
      trackerRef.current?.track(now ? 'FULLSCREEN_ENTERED' : 'FULLSCREEN_EXITED');
    };
  ...
  return { isFullscreen, requestFullscreen, blocked, dismissBlocked: () => setBlocked(null), report };
}
