...
import {
  Play, Send, CheckCircle, XCircle, Trophy,
  BookOpen, History, List, Loader2, AlertTriangle, Flag,
  Terminal, RotateCcw, Maximize, Minimize, ShieldAlert
} from 'lucide-react';
  ...
  return (
    <div className="relative h-full flex flex-col bg-muted/30 dark:bg-[#0d1117] rounded-none overflow-hidden">
      ...
      <div className="h-6 border-t border-border bg-muted/50 dark:bg-[#161b22] flex items-center px-4 gap-4 shrink-0">
        <span className="text-[11px] text-muted-foreground font-mono">{language.toUpperCase()}</span>
        <span className="text-[11px] text-muted-foreground font-mono">{value.split('\n').length} lines</span>
        <span className="text-[11px] text-muted-foreground font-mono">UTF-8</span>
      </div>
    </div>
  );
}
...
export default function ContestWorkspacePage() {
  ...
  const { data: session, isLoading: sessionLoading } = useQuery({
    ...
    queryFn: () => api.get<{ started: boolean; expiresAt?: string; status?: string }>(`/contests/${contestId}/session`),
    enabled: !!contestId,
  });
  ...
  const { data: mySubmissions = [] } = useQuery({
    ...
    queryFn: () => api.get<Submission[]>(`/contests/${contestId}/my-submissions`),
    enabled: !!contestId && !!session?.started,
  ...
  const { data: leaderboardEntries = [] } = useQuery({
    ...
    queryFn: () => api.get<LeaderboardEntry[]>(`/contests/${contestId}/leaderboard`),
    enabled: !!contestId && !!session?.started,
  ...
  const activeStage = selectedProblem?.isProgressive
    ? selectedProblem.stages?.find(s => s.stageOrder === selectedProblem.currentStageOrder)
    : undefined;

  // Solved problems tracking (derived from the best submission per problem)
  const solvedProblems = new Set(
    mySubmissions.filter(s => s.status === 'ACCEPTED').map(s => s.problemId)
  );
  ...
  const { data: notifications = [] } = useQuery({
    ...
    queryFn: () => api.get<ContestNotice[]>(`/contests/${contestId}/notifications`),
    enabled: !!contestId && !!session?.started,
    refetchInterval: 20000,
  });
  ...
  useEffect(() => {
    ...
    setSeenNotifications(prev => new Set([...prev, ...unseen.map(n => n.id)]));
  ...
  const expiresAt = session?.expiresAt ?? new Date(Date.now() + 60 * 60 * 1000).toISOString();
  ...
  useEffect(() => {
    ...
    autosaveTimer.current = window.setTimeout(async () => {
      ...
      try {
        await api.put(`/contests/${contestId}/problems/${selectedProblem.id}/draft`, { language, sourceCode: code });
        setSaveStatus('saved');
      } catch {
        setSaveStatus('unsaved');
      }
  ...
  const handleProblemSelect = async (problem: Problem) => {
    ...
    if (!contestId) return;
    try {
      const draft = await api.get<{ language: Language; sourceCode: string } | null>(
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const result = await api.post<{
        ...
        problemId: selectedProblem.id, contestId, stageId: activeStage?.id, language, code, stdin: runInput,
      });
      setRunOutput({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await api.post<{
        status: Verdict; passedTests: number; totalTests: number; score: number; compileOutput?: string;
        testResults: { name: string; hidden: boolean; passed: boolean; status: string; executionTime: number; errorMessage?: string | null }[];
      }>('/submissions', { problemId: selectedProblem.id, contestId, stageId: activeStage?.id, language, code });

      if (result.status === 'ACCEPTED') {
        if (activeStage) {
          const isLastStage = activeStage.stageOrder === (selectedProblem.totalStages ?? activeStage.stageOrder);
          toast.success(isLastStage ? '🏆 Chain complete! All stages solved.' : '✅ Stage cleared — next challenge unlocked!');
        } else {
          toast.success('✅ Accepted! All test cases passed!');
        }
      } else {
        toast.info(`${result.passedTests}/${result.totalTests} test cases passed`);
      }

      setRunOutput({
        stdout: '', stderr: '', compileOutput: result.compileOutput || '', status: result.status,
        testResults: result.testResults.map((tr, i) => ({
          id: String(i), label: tr.name, status: tr.passed ? 'passed' : 'failed',
          executionTime: Math.round(tr.executionTime * 1000), errorMessage: tr.errorMessage,
        })),
      });
      ...
      toast.error(err instanceof ApiError ? err.message : 'Failed to submit solution');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      toast.error(err instanceof ApiError ? err.message : 'Could not finish the contest');
    }
  };
    ...
    <div className="h-screen flex flex-col bg-background overflow-hidden">
      ...
      <div className="h-12 border-b border-border bg-card flex items-center px-4 gap-4 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <ContestTimer expiresAt={expiresAt} onExpire={() => toast.error('Time is up! Contest auto-completed.')} />
          <Button
            size="sm"
            variant="outline"
        ...
        <div className="w-52 shrink-0 border-r border-border bg-card flex flex-col">
          <div className="p-3 border-b border-border">
            <div className="flex gap-1">
              {(['problem', 'submissions', 'leaderboard'] as const).map(tab => {
                const icons = { problem: List, submissions: History, leaderboard: Trophy };
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {activeTab === 'problem' && problems.map((p, i) => {
                ...
                <button key={p.id} onClick={() => handleProblemSelect(p)}
                  ...
                  <div className="flex items-center gap-2 mb-0.5">
                    {solved
                      ...
                      : <div className={`w-3 h-3 rounded-full border shrink-0 ${isSelected ? 'border-primary' : 'border-border'}`} />
                    }
                    <span className={`text-xs font-semibold truncate ${isSelected ? 'text-primary' : solved ? 'text-success' : 'text-foreground'}`}>
                      {i + 1}. {p.title}
                    </span>
                  </div>
                  <div className="flex items-center justify-between pl-5">
                    ...
                    {p.isProgressive ? (
                      <span className="text-[10px] text-primary font-mono">
                        Stage {Math.min(p.currentStageOrder ?? 1, p.totalStages ?? 1)}/{p.totalStages}
                      </span>
                    ) : (
                      <span className="text-[10px] text-muted-foreground font-mono">{p.maxScore}p</span>
                    )}
                  </div>
                      ...
                      {p.stages.map(s => (
                        <span key={s.id} title={s.title}
                          className={`w-1.5 h-1.5 rounded-full ${
                            s.locked ? 'bg-border' : s.stageOrder < (p.currentStageOrder ?? 1) ? 'bg-success' : 'bg-primary'
                          }`} />
                      ))}
                    </div>
                  )}
            ...
            {activeTab === 'submissions' && (
              <div className="space-y-2 pt-1">
                ...
                {mySubmissions.map(s => (
                  <div key={s.id} className="p-2 rounded-lg bg-muted border border-border">
                    <div className="text-[11px] font-medium text-foreground truncate mb-1">{s.problemTitle}</div>
                    <VerdictBadge status={s.status} />
                    <div className="flex justify-between mt-1">
                      <span className="text-[10px] text-muted-foreground font-mono">{s.passedTests}/{s.totalTests}</span>
                      <span className="text-[10px] text-muted-foreground font-mono">{s.language.toUpperCase()}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {activeTab === 'leaderboard' && (
              <div className="space-y-1 pt-1">
                {leaderboardEntries.map(e => (
                  <div key={e.userId} className="px-2 py-2 rounded-lg hover:bg-muted transition-colors">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-bold w-5 text-center font-mono ${
                        e.rank === 1 ? 'text-warning' : e.rank === 2 ? 'text-muted-foreground' : e.rank === 3 ? 'text-amber-600' : 'text-muted-foreground'
                      }`}>{e.rank}</span>
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-medium text-foreground truncate">{e.userName}</div>
                        <div className="text-[10px] text-muted-foreground font-mono">{e.solved}/{e.totalProblems} solved</div>
                      </div>
                      <span className="text-xs font-bold text-primary font-mono">{e.score}</span>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <>
                    <div className="px-5 py-4 border-b border-border shrink-0">
                      <h2 className="font-bold text-base text-foreground mb-1">
                        ...
                        {activeStage && <span className="text-primary"> — {activeStage.title}</span>}
                      </h2>
                      <div className="flex items-center gap-3">
                        <DifficultyBadge difficulty={selectedProblem.difficulty} />
                        <span className="text-xs text-muted-foreground font-mono">
                          ⏱ {activeStage?.timeLimit ?? selectedProblem.timeLimit}s · 💾 {activeStage?.memoryLimit ?? selectedProblem.memoryLimit}MB
                        </span>
                      ...
                      {selectedProblem.isProgressive && (
                        <p className="text-[11px] text-muted-foreground mt-2">
                          Stage {activeStage?.stageOrder ?? 1} of {selectedProblem.totalStages} — solve and submit to unlock the next enhancement.
                        </p>
                      )}
                    </div>
                    <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5 text-sm text-foreground">
                      <div>
                        ...
                        <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">
                          {activeStage?.statement || selectedProblem.description}
                        </p>
                      </div>
                      {!activeStage && (
                        <>
                          <div>
                            <h3 className="text-xs font-semibold text-primary uppercase tracking-wider mb-2">Input Format</h3>
                            <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">{selectedProblem.inputFormat}</p>
                          </div>
                          <div>
                            <h3 className="text-xs font-semibold text-primary uppercase tracking-wider mb-2">Output Format</h3>
                            <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">{selectedProblem.outputFormat}</p>
                          </div>
                        </>
                      )}
                      <div>
                        <h3 className="text-xs font-semibold text-primary uppercase tracking-wider mb-2">Constraints</h3>
                        <pre className="text-muted-foreground font-mono text-xs leading-relaxed whitespace-pre-wrap bg-muted p-3 rounded-lg border border-border">{selectedProblem.constraints}</pre>
                      </div>
                      {activeStage?.testCases?.filter(tc => !tc.hidden).map((tc, i) => (
                        <div key={tc.id}>
                          <h3 className="text-xs font-semibold text-primary uppercase tracking-wider mb-2">Sample {i + 1}</h3>
                          <div className="space-y-2">
                            <div className="bg-muted border border-border rounded-lg p-3">
                              <div className="text-[10px] text-muted-foreground font-mono mb-1">INPUT</div>
                              <pre className="text-xs font-mono text-foreground whitespace-pre-wrap">{tc.input}</pre>
                            </div>
                            <div className="bg-muted border border-border rounded-lg p-3">
                              <div className="text-[10px] text-muted-foreground font-mono mb-1">EXPECTED OUTPUT</div>
                              <pre className="text-xs font-mono text-foreground whitespace-pre-wrap">{tc.expectedOutput}</pre>
                            </div>
                          </div>
                        </div>
                      ))}
                      {!activeStage && selectedProblem.examples.map((ex, i) => (
                        <div key={i}>
                          <h3 className="text-xs font-semibold text-primary uppercase tracking-wider mb-2">Example {i + 1}</h3>
                          <div className="space-y-2">
                            <div className="bg-muted border border-border rounded-lg p-3">
                              <div className="text-[10px] text-muted-foreground font-mono mb-1">INPUT</div>
                              <pre className="text-xs font-mono text-foreground whitespace-pre-wrap">{ex.input}</pre>
                            </div>
                            <div className="bg-muted border border-border rounded-lg p-3">
                              <div className="text-[10px] text-muted-foreground font-mono mb-1">OUTPUT</div>
                              <pre className="text-xs font-mono text-foreground whitespace-pre-wrap">{ex.output}</pre>
                            </div>
                            {ex.explanation && (
                    ...
                    <div className="h-9 border-b border-border bg-card flex items-center px-3 gap-3 shrink-0">
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => { setCode(selectedProblem?.boilerplates[language] ?? ''); toast.info('Reset to boilerplate'); }}
                ...
                <ResizablePanel defaultSize={35} minSize={15}>
                  <Tabs value={bottomTab} onValueChange={(v) => setBottomTab(v as 'output' | 'stdin')} className="h-full flex flex-col bg-background">
                    ...
                    <div className="flex-1 overflow-y-auto p-3">
                      ...
                      {bottomTab === 'output' && !runOutput && !running && !submitting && (
                        <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                          <Terminal size={24} className="mb-2 opacity-40" />
                          <p className="text-xs">Run or submit to see output</p>
                        </div>
                      )}
                      ...
                      {bottomTab === 'output' && runOutput && (
                        <div className="space-y-3">
                          ...
                          {runOutput.testResults.length > 0 && (
                            <div>
                              <div className="text-[10px] text-muted-foreground font-mono mb-2 uppercase tracking-wider">
                                Test Cases — {runOutput.testResults.filter(r => r.status === 'passed').length}/{runOutput.testResults.length} Passed
                              </div>
                                ...
                                {runOutput.testResults.map((r, i) => (
                                  <div key={r.id}
                                    ...
                                    {r.status === 'passed'
                                      ...
                                      : <XCircle size={10} />}
                                    <span>#{i + 1}</span>
                                    {r.executionTime !== undefined && <span className="opacity-70">{r.executionTime}ms</span>}
                                  </div>
                                ))}
                              </div>
                            </div>
                          ...
                          {runOutput.status && runOutput.testResults.length === 0 && (
                          ...
                          {runOutput.compileOutput && (
                            <div>
                              <div className="text-[10px] text-warning font-mono mb-1 uppercase tracking-wider">Compile Output</div>
                              <pre className="text-xs font-mono text-warning bg-warning/5 p-2 rounded border border-warning/20 whitespace-pre-wrap">{runOutput.compileOutput}</pre>
                            </div>
                          )}

                          {runOutput.stdout && (
                            <div>
                              <div className="text-[10px] text-muted-foreground font-mono mb-1 uppercase tracking-wider">Stdout</div>
                              <pre className="text-xs font-mono text-foreground bg-muted p-2 rounded border border-border whitespace-pre-wrap">{runOutput.stdout}</pre>
                            </div>
                          )}

                          {runOutput.stderr && (
                            <div>
                              <div className="text-[10px] text-destructive font-mono mb-1 uppercase tracking-wider">Stderr</div>
                              <pre className="text-xs font-mono text-destructive bg-destructive/5 p-2 rounded border border-destructive/20 whitespace-pre-wrap">{runOutput.stderr}</pre>
                            </div>
                          )}
                        </div>
                      )}
        ...
        <DialogContent className="bg-card border-border">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <AlertTriangle size={18} className="text-warning" /> Finish Contest?
            </DialogTitle>
            <DialogDescription className="text-muted-foreground">
              You still have time remaining. You will not be able to modify your solutions after finishing.
              Are you sure you want to finish?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowFinishDialog(false)}>Cancel</Button>
            <Button className="bg-destructive hover:bg-destructive/90 text-destructive-foreground" onClick={handleFinish}>
              Finish Contest
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!blocked} onOpenChange={open => !open && dismissBlocked()}>
        <DialogContent className="bg-card border-border max-w-sm" onEscapeKeyDown={e => e.preventDefault()}>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <ShieldAlert size={18} className="text-warning" /> {blocked?.title}
            </DialogTitle>
            <DialogDescription className="text-muted-foreground">{blocked?.message}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button className="btn-primary" onClick={dismissBlocked}>Got it</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <div className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="card-glow rounded-xl p-8 max-w-md text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              This contest can only be taken in fullscreen mode. Leaving fullscreen has been recorded.
              Return to fullscreen to continue — your timer is still running.
            </p>
