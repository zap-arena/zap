...
export default function AdminAnalytics() {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortField === "name") {
        ...
        valB = MALPRACTICE_LABELS.reduce(
          (sum, label) => sum + (b.metrics[label] || 0),
          0,
        );
      } else {
  ...
  const exportCSV = () => {
    ...
    const rows = filteredAndSorted.map((row) => {
      const totalEvents = Object.values(row.metrics).reduce(
        (sum, c) => sum + c,
        0,
      );
      const malpracticeScore = MALPRACTICE_LABELS.reduce(
        (sum, label) => sum + (row.metrics[label] || 0),
        0,
      );
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-0.5">
              Track user activity, overall metrics, and candidate malpractice.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 bg-card border border-border p-2 rounded-xl shadow-sm">
            ...
            <Button
              ...
              size="sm"
              onClick={exportCSV}
              className="gap-2 h-9 border-border text-foreground hover:bg-muted"
            >
        ...
        <Tabs defaultValue="global" className="w-full">
          <TabsList className="mb-4">
            ...
            <TabsTrigger value="users" className="flex items-center gap-2">
              <Activity size={14} /> Candidate Details
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="global" className="space-y-6">
            ...
            <div className="card-glow rounded-xl overflow-hidden bg-card border border-border p-5">
              ...
              <div className="w-full h-[400px]">
                {loadingEvents ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-muted-foreground text-sm">
                    No tracking data available to graph.
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                      data={eventsData.timeseries}
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs text-muted-foreground">
                  {eventsData?.summary.length || 0} unique labels
                </span>
              </div>
              ...
              {loadingEvents ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {eventsData.summary.map((row) => (
                    <div
                      key={row.label}
                      className="card-glow rounded-xl p-4 flex flex-col justify-between"
                    >
                ...
                <table className="w-full data-table">
                  <thead>
                    <tr>
                      ...
                      {dynamicLabels.map((label) => (
                        <th
                          key={label}
                          className="text-center text-xs font-semibold text-muted-foreground whitespace-nowrap px-4"
                        >
                          {label}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {loadingUsers ? (
                      <tr>
                        <td
                          colSpan={4 + dynamicLabels.length}
                        ...
                        return (
                          ...
                          >
                            ...
                            {dynamicLabels.map((label) => {
                              ...
                              const isMalpractice =
                                MALPRACTICE_LABELS.includes(label);
