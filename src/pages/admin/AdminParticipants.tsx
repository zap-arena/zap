...
export default function AdminParticipants() {
  ...
  const { data: participants = [] } = useQuery({
    ...
    queryFn: async () => {
      const lists = await Promise.all(contests.map(c => api.get<Participant[]>(`/admin/contests/${c.id}/participants`)));
      return lists.flat();
    },
    ...
    <AdminLayout>
      <div className="p-6 max-w-7xl mx-auto space-y-5 animate-fade-in">
        <div>
          ...
          <p className="text-muted-foreground text-sm">{participants.length} participants across all contests</p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            { label: 'In Progress', value: participants.filter(p => p.status === 'in_progress').length, color: 'text-info' },
            { label: 'Completed', value: participants.filter(p => p.status === 'completed' || p.status === 'auto_completed').length, color: 'text-success' },
            { label: 'Not Started', value: participants.filter(p => p.status === 'not_started').length, color: 'text-muted-foreground' },
          ...
          <table className="w-full data-table">
            <thead>
              <tr>
                ...
                <th className="text-center hidden sm:table-cell">Submissions</th>
                ...
                return (
                  <tr key={p.id} className="group">
                    ...
                    <td className="text-center">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase tracking-wide ${STATUS_STYLES[p.status]}`}>
                        {p.status.replace('_', ' ')}
                      </span>
                    </td>
                    <td className="text-center font-mono font-bold text-primary">{p.score}</td>
                    <td className="text-center text-xs text-muted-foreground hidden sm:table-cell font-mono">{p.problemsSolved}</td>
                    <td className="text-center text-xs text-muted-foreground hidden sm:table-cell font-mono">{p.totalSubmissions}</td>
          ...
          <DialogContent className="bg-card border-border max-w-lg max-h-[80vh] overflow-y-auto">
            ...
            {selected && (
              <div className="space-y-4 py-2 text-sm">
                ...
                <div>
                  <p className="font-semibold mb-2 text-xs text-muted-foreground uppercase tracking-wider">Submission History</p>
                  {subs.length === 0
                    ? <p className="text-muted-foreground text-xs">No submissions</p>
                    : (
                      <div className="space-y-2">
                        {subs.map(s => (
                          <div key={s.id} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30 border border-border">
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-medium truncate">{s.problemTitle}</p>
                              <p className="text-[10px] text-muted-foreground">{s.language.toUpperCase()} · {s.passedTests}/{s.totalTests} tests</p>
                            </div>
                            <span className={`text-[10px] px-1.5 py-0.5 rounded font-semibold font-mono ${
                              s.status === 'ACCEPTED' ? 'text-success bg-success/15' : 'text-destructive bg-destructive/15'
                            }`}>{s.score}pts</span>
                          </div>
                        ))}
                      </div>
