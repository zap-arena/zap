...
export default function AdminLogs() {
  ...
  const avgDuration =
    logs.length > 0
      ? Math.round(
          (logs.reduce((s, l) => s + l.executionDuration, 0) / logs.length) *
            1000,
        )
      : 0;
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-5 animate-fade-in">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Backend execution audit trail — {logs.length} records
          </p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            <div key={label} className="card-glow rounded-xl p-4 text-center">
              <div className={`text-2xl font-bold mb-0.5 font-mono ${color}`}>
                {value}
              </div>
            ...
            <tbody>
              {filtered.map((log) => (
                <tr key={log.id} className="font-mono">
                  ...
                  <td className="text-center text-xs uppercase">
                    {log.language}
                  </td>
                  <td className="text-center">
                    <VerdictBadge status={log.status} />
                  </td>
                  ...
                  <td className="text-right text-xs text-primary">
