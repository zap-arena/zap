...
import type { Contest, ParticipantAnalytics, BehaviorPattern } from '../../types';
...
export default function ProgressiveAnalyticsPage() {
  ...
  const { data: participants = [], isLoading } = useQuery({
    ...
    queryFn: () => api.get<ParticipantAnalytics[]>(`/admin/contests/${id}/progressive-analytics`),
    enabled: !!id,
  });
    ...
    <AdminLayout>
      <div className="p-6 max-w-6xl mx-auto space-y-5 animate-fade-in">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate(`/admin/contests/${id}`)} className="h-8 w-8">
            <ArrowLeft size={16} />
          </Button>
          <div className="flex-1">
            <h1 className="text-2xl font-bold flex items-center gap-2">
              <Brain size={20} className="text-primary" /> Solving Behavior Analytics
            </h1>
            <p className="text-muted-foreground text-sm">
              {contest?.name ?? 'Progressive contest'} — how each student approached the chain (admin-only, does not affect scores).
            </p>
          </div>
        </div>
          ...
          <div key={p.userId} className="card-glow rounded-xl overflow-hidden">
            ...
            >
              <div className="text-left">
                ...
                <div className="text-xs text-muted-foreground">{p.chains.length} chain(s) attempted</div>
              </div>
              <div className="flex items-center gap-3">
                {p.chains.map(c => (
                  <span key={c.problemId} className={`text-[10px] px-2 py-1 rounded-full font-mono ${PATTERN_COLOR[c.pattern]}`}>
                    {PATTERN_LABEL[c.pattern]} · score {c.behaviorScore}
                  </span>
                ))}
                {expanded.has(p.userId) ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
              </div>
            </button>
                ...
                {p.chains.map(chain => (
                  <div key={chain.problemId} className="px-5 py-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-semibold">{chain.problemTitle}</h4>
                      <span className={`text-xs px-2 py-1 rounded-full font-mono ${PATTERN_COLOR[chain.pattern]}`}>
                      ...
                      <tbody>
                        {chain.stages.map(s => (
                          <tr key={s.stageId}>
                            ...
                            <td className="text-center">{s.solved ? '✓' : '—'}</td>
                            <td className="text-center font-mono">{s.attempts}</td>
                            <td className="text-center font-mono">{s.runs}</td>
                            <td className="text-center font-mono">{s.errorsResolved}/{s.errorsSeen.length}</td>
                            <td className="text-center font-mono">
                              {s.timeToSolveSeconds != null ? `${Math.round(s.timeToSolveSeconds)}s` : '—'}
                            </td>
                            <td className="text-center font-mono">{Math.round(s.codeChurn * 100)}%</td>
                            <td className="text-center font-mono">
                              {s.complexity?.label ?? '—'} ({s.expectedComplexity ?? '?'})
                            </td>
                          </tr>
                        ))}
                      </tbody>
