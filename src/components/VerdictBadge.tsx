...
const config: Record<Verdict, { label: string; className: string }> = {
  ...
  TIME_LIMIT_EXCEEDED: { label: 'Time Limit Exceeded', className: 'verdict-error' },
  MEMORY_LIMIT_EXCEEDED: { label: 'Memory Limit Exceeded', className: 'verdict-error' },
...
export default function VerdictBadge({ status }: { status: Verdict }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold font-mono ${className}`}>
      {label}
    </span>
  );
}
